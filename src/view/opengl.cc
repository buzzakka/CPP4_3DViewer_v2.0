...
void OpenGL::paintGL() {
  glClearColor(main_color_.redF(), main_color_.greenF(), main_color_.blueF(),
               main_color_.alphaF());
...
void OpenGL::Perspective() {
  ...
  glLoadIdentity();
  if (is_parallel_) {
    glOrtho(-(data_.max_coord), data_.max_coord, -(data_.max_coord),
            data_.max_coord, -(data_.far), data_.far);
  } else {
    glFrustum(-(data_.max_coord), data_.max_coord, -(data_.max_coord),
              data_.max_coord, data_.near, data_.far);
...
void OpenGL::Draw() {
  ...
  glDrawElements(GL_LINES, data_.facets.size(), GL_UNSIGNED_INT,
                 data_.facets.data());
